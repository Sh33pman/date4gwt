...
//  }
     ...
     */
    static int hashCodeFor(Integer... aFields) {
        int result = HASH_SEED;
        for (Integer field : aFields) {
            result = hash(result, field == null ? 0 : field.intValue());
        }
        return result;
    }
...
//  }
    ...
    static boolean equalsFor(Object[] aThisSignificantFields, Object[] aThatSignificantFields) {
        ...
        for (int idx = 0; idx < aThisSignificantFields.length; ++idx) {
            if (!equals(aThisSignificantFields[idx], aThatSignificantFields[idx])) {
                result = false;
                break;
            }
        }
    ...
    /**
     * Ported from Java 7 Objects.equals
