...
final class DateTimeFormatter {
    ...
    private static final RegExp ESCAPED_RANGE = RegExp.compile("\\|[^\\|]*\\|");
    ...
    private static final RegExp FRACTIONALS = RegExp.compile("f{1,9}");
    ...
    String format(DateTime aDateTime) {
        ...
        findEscapedRanges();
        System.out.println("findEscapedRanges();");
    ...
    private String interpretThe(String aCurrentToken, DateTime aDateTime) {
        ...
        if (YYYY.equals(aCurrentToken)) {
            ...
            MatchResult matcher = FRACTIONALS.exec(aCurrentToken);
            if (matcher != null) {
    ...
    private String lookupMonthFor(Integer aMonth) {
        ...
        if (!fMonths.containsKey(fLocale)) {
            ...
            DateTimeFormat format = DateTimeFormat.getFormat("MMMM");
            for (int idx = 0; idx <= 11; ++idx) {
                Date firstDayOfMonth = new Date();
                firstDayOfMonth.setYear(2000);
                firstDayOfMonth.setMonth(idx);
                firstDayOfMonth.setDate(15);
                String monthText = format.format(firstDayOfMonth);
    ...
    private String lookupWeekdayFor(Integer aWeekday) {
        ...
        if (!fWeekdays.containsKey(fLocale)) {
            ...
            for (int idx = 8; idx <= 14; ++idx) {
                ...
                firstDayOfWeek.setYear(2009);
                firstDayOfWeek.setMonth(1); //month is 0-based
                firstDayOfWeek.setDate(idx);
                String weekdayText = format.format(firstDayOfWeek);
    ...
    private String getAmPmTextFor(Integer aHour) {
        DateTimeFormat format = DateTimeFormat.getFormat("a");
        Date someDay = new Date();
        someDay.setYear(2000);
        someDay.setMonth(6);
        someDay.setDate(15);
        someDay.setHours(aHour);
        return format.format(someDay);
    }
