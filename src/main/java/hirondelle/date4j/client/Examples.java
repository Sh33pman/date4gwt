...
public final class Examples {
  ...
  */
  ...
  private void currentDateTime(){
    DateTime now = DateTime.now(TimeZone.createTimeZone(0));
  ...
  private void currentDateTimeInCairo(){
    DateTime now = DateTime.now(TimeZone.createTimeZone("Africa/Cairo"));
    String result = now.format("YYYY-MM-DD hh:mm:ss (WWWW)", LocaleInfo.getCurrentLocale());
    log("Current date-time in Cairo : " + result);
  }
  
  /** What's the age of someone born May 16, 1995? */
  private void ageIfBornOnCertainDate(){
    DateTime today = DateTime.today(TimeZone.createTimeZone(0));
  ...
  private void hoursDifferenceBetweenParisAndPerth(){
    ...
    DateTime paris = DateTime.now(TimeZone.createTimeZone("Europe/Paris"));
    DateTime perth = DateTime.now(TimeZone.createTimeZone("Australia/Perth"));
