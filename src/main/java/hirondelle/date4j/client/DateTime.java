...
public final class DateTime implements Comparable<DateTime>, Serializable {
    ...
    public static DateTime forInstant(long aMilliseconds, TimeZone aTimeZone) {
        Date date = new Date();
        date.setTime(aMilliseconds + 1000 * 60 * aTimeZone.getOffset(date) - date.getTimezoneOffset() * 60 * 1000);
     ...
     * <p/>
     * <P>If months, weekdays, or AM/PM indicators are output as localizable text, you must use {@link #format(String)}.
     *
  ...
  /* The following 7 items represent the parsed form of a DateTime. */
     ...
     * <p/>
     * <P>Use of this method is likely relatively rare; it should be used only if the output of {@link #format(String)}  is
     * inadequate.
     *
     * @param aFormat         uses the <a href="#FormattingLanguage">formatting mini-language</a> defined in the class comment.
    ...
    public DateTime changeTimeZone(TimeZone aFromTimeZone, TimeZone aToTimeZone) {
        ...
        fromDate.setYear(getYear() - 1900);
        fromDate.setMonth(getMonth() - 1);
        fromDate.setDate(getDay());
        fromDate.setHours(getHour());
        if (getMinute() != null) {
            fromDate.setMinutes(getMinute());
        } else {
